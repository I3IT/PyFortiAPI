...
class FortiGate:
    ...
    def trigger_webhook_ban_ip(self, webhook_name, token, ip, expiry):
        ...
        :param webhook_name: Name of the webhook to trigger IP ban/Quarantine
        :param ip: IP to ban
        ...
        #result = requests.put(api_url, headers=headers, json=data, verify=self.verify, timeout=self.timeout).status_code
        #self.logout(session)
        return result
